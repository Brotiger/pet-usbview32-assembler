...
INCLUDELIB lib/import32.lib	; Работа с библиотекой ОС Kernel32.
...
TDeviceName STRUC	
   ...
   Name  	dw 256 dup (?)	; Символьное имя устройства в Юникоде
TDeviceName ENDS	
...
THubInformation STRUC	
   ...
   HubIsBusPowered	db ?  	; Способ питания хаба: от шины (1) ;или от внешнего источника (0)
THubInformation ENDS	

; Структура запроса/ответа с информацией о хабе
TNodeInformation STRUC	
   NodeType	      dd ? 		; Тип устройства: 0 - хаб ;1 - комбинир. устройство
...
TDeviceDescriptor STRUC	
   ...
   bMaxPacketSize	db ?	; Макс. размер пакета для нулевой ;конечной точки
   idVendor      	dw ?	; Идентификатор производителя
   idProduct     	dw ?	; Идентификатор продукта
   bcdDevice     	dw ?	; Номер версии устройства
   iManufacturer	db ?	; Индекс дескриптора строки, ;описывающего производителя
   iProduct      	db ?	; Индекс дескриптора строки, ;описывающего продукт
   iSerialNumber	db ?	; Индекс дескриптора строки с серийным ;номером устройства
...
TNodeConnectionInformation STRUC
   ...
   Speed     			db ? 	     	; Скорость работы устройства ;(0-LS, 1-FS, 2-HS, 3-SS)
   ...
   PipeList    			db 32 dup (?)	; Массив структур с описание ;каналов, связанных с портом
...
; Выводимые сообщения	
  ...
  msgEndPointDescr db 0B3h,'  ',0B3h, 13, 10
			...
			db 0B3h,'  ',0B3h,' bEndpointAddress = %u', 13, 10
			db 0B3h,'  ',0B3h,'	bmAttributes = %u', 13, 10
...
; Получение дескрипторов интерфейса
	...
	mov 	DescriptorRequest.SetupPacket.bmRequest, 80h
	mov 	DescriptorRequest.SetupPacket.bRequest, USB_REQUEST_GET_DESCRIPTOR
	mov 	DescriptorRequest.SetupPacket.wValue, USB_INTERFACE_DESCRIPTOR_TYPE
	shl 	DescriptorRequest.SetupPacket.wValue, 8
	mov 	DescriptorRequest.SetupPacket.wLength, 100h
...
; Получение дескрипторов конечной точки
	...
	mov 	DescriptorRequest.SetupPacket.bmRequest, 80h
	mov 	DescriptorRequest.SetupPacket.bRequest, USB_REQUEST_GET_DESCRIPTOR
	mov 	DescriptorRequest.SetupPacket.wValue, USB_ENDPOINT_DESCRIPTOR_TYPE
	shl 	DescriptorRequest.SetupPacket.wValue, 8
	mov 	DescriptorRequest.SetupPacket.wLength, 100h
...
DesInterface:	
	cmp 	byte ptr [edi+1], 4	; Дескриптор конфигурации?
	jne 	DesEndPoint	
	xor 	eax, eax
	mov 	al, [edi+8]	
	push 	eax		
...
DesEndPoint:	
	cmp 	byte ptr [edi+1], 5	; Дескриптор конфигурации?
	jne 	NextDescr	
	xor 	eax, eax
	mov 	al, [edi+6]	
	push 	eax
...
mWait: 				; Ожидание нажатия клавиши
	...
	jmp 	mWait	; Событие от клавиатуры?
END Start